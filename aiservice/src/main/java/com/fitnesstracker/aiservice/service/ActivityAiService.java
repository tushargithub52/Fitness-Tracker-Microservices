...
public class ActivityAiService {
    ...
    private Recommendation processAiResponse(Activity activity, String aiResponse) {
        try {
            ...
            // clean json response
            String jsonResponse = textNode.asString()
                    .replaceAll("\\n```", "")
                    .replaceAll("```json\\n", "")
                    .trim();

//            log.info("Cleaned ai response : {}", jsonResponse);

            JsonNode analysisJson = objectMapper.readTree(jsonResponse);
    ...
    Overall: This is an excellent workout
     */
    private void addAnalysisSection(StringBuilder fullAnalysis, JsonNode analysisNode, String aiResponseKey, String prefix) {
        if (!analysisNode.path(aiResponseKey).isMissingNode()) {
            fullAnalysis.append(prefix)
                    .append(analysisNode.path(aiResponseKey).asString())
                    .append("\n\n");
        }
    }
